...
matplotlib.use('Agg')
epoch = 80
attention_node_path   = '/home/weihsin/projects/MotionExpert/STAGCN_output_finetune/att_node_results_epoch'+str(epoch)+'.json'
attention_matrix_path = '/home/weihsin/projects/MotionExpert/STAGCN_output_finetune/att_A_results_epoch'+str(epoch)+'.json'
...
def draw(joints,video_path,output_path,file_name):    
    ...
    while True:
        ...
        if not _: break
        concatenate_frame = []
        for attention_id in [0,2,3]:
            color_node,attention_matrix_new = readattention(file_name,num_frame,attention_id) 
            frame = frame_c.copy()
            if attention_id == 0 :
            ...
            for point in points_list:
                ...
                if attention_id == 0 or attention_id == 3 :
                    color_map = cv2.applyColorMap(np.array([[0 + 127 * size]]).astype(np.uint8),COLORMAP)
                else :
                    color_map = cv2.applyColorMap(np.array([[255 - 127 * size]]).astype(np.uint8),COLORMAP)
                rgb_value = (int(color_map[0, 0, 0]), int(color_map[0, 0, 1]), int(color_map[0, 0, 2]))
                if (size  < 3) :
                    cv2.circle(frame, (int(point[0]), int(point[1])), int(4-round(size,1)), rgb_value, -1)
                point_id += 1
            ...
            for index in top3:
                ...
                if attention_id == 0 or attention_id == 3:   scaled_value_uint8 = int((1-index[0]) * 255)
                else :                                       scaled_value_uint8 = int((index[0]) * 255)
    ...
    ###########################################################################################################################   
        ...
        if(frame_index == num_frame) : break
...
output_folder = '/home/weihsin/projects/Evaluation/Video/epoch'+str(epoch)
